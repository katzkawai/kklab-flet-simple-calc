...
Flet のギャラリーからサンプルアプリを入手して、自分好みに改造し、
...
> 💻 **コマンドの実行環境について（このチュートリアルは macOS / Linux 向けに書いています）**
> `uv` や `flet`（`uv sync` / `uv run flet run` / `uv run flet build web`）は **Windows でもそのまま動きます**。
> ただし、公開（デプロイ）で使う `cp`・`rm`・`touch` などのシェルコマンドや `./deploy.sh` は
> **macOS / Linux のシェル（bash / zsh）向け** で、**Windows の PowerShell ではそのままでは動きません**。
> Windows の場合は、次のいずれかで進めてください。
> 1. **コーディングエージェントに任せる**（OS を問わず動く・いちばん簡単 → 後述の「🤖 5-1 の別案」）
> 2. **Git Bash** または **WSL** を開いて、このチュートリアルのコマンドや `./deploy.sh` をそのまま実行する
> 3. **PowerShell 版のコマンド**を使う（後述のステップ 5-1 に併記してあります）
...
- `uv sync` … `pyproject.toml` を読んで必要なライブラリを `.venv` に揃えます（`uv.lock` があればそのバージョンに固定。無ければ自動で作成されます）。
...
#### 💻 Windows（PowerShell）で公開する場合

上の 5-1 のコマンドは macOS / Linux 向けです（`cp`・`rm`・`touch` などは Unix のコマンド）。
Windows で **PowerShell** を使うなら、同じことを行う下のコマンドに読み替えてください。
ビルド（`uv run flet build web ...`）は OS 共通で、**その後の公開部分だけ**が変わります。

```powershell
# 0. リポジトリのルートを覚えておく（最後に戻るため）
$repo = Get-Location
$site = Join-Path $env:TEMP "ghp-site"
$work = Join-Path $env:TEMP "ghp"

# 1. ビルド結果を一時フォルダにコピー
if (Test-Path $site) { Remove-Item -Recurse -Force $site }
Copy-Item -Recurse build\web $site
...
Copy-Item -Recurse -Force (Join-Path $site "*") .
git add -A
git commit -m "Deploy web app to GitHub Pages"
git push -f origin gh-pages
...
> 💡 PowerShell に読み替えるのが面倒なら、**Git Bash や WSL** を開いて 5-1 のコマンド（または `./deploy.sh`）を
> そのまま実行する手もあります。いちばん簡単なのは、次の「🤖 5-1 の別案」（OS を問わず動く）です。
...
> 💻 `deploy.sh` は **bash スクリプト** です。Windows の PowerShell からは直接実行できないので、
> **Git Bash** か **WSL** を開いて `./deploy.sh`（または `bash deploy.sh`）を実行してください。
